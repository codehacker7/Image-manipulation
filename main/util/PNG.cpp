...
namespace cs221util {
  ...
  bool PNG::operator==(PNG const & other) const {
    if (width_ != other.width_) { cout << "width problem" << endl;
      return false; }
    if (height_ != other.height_) {cout << "height problem" << endl;
    return false; }
  ...
  HSLAPixel * PNG::getPixel(unsigned int x, unsigned int y) const {
    if (width_ == 0 || height_ == 0) {
      cerr << "ERROR: Call to cs221::PNG::getPixel() made on an image with no pixels." << endl;
      assert(width_ > 0);
      assert(height_ > 0);
    }
